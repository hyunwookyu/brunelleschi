...
## 5단계 진입 전 정지 (§13 개정, 2026-08-12)
4단계 완료. **5단계(관계 추론) 진입 전 정지.** 재개 조건: unmappable 원장에 승격 판단용
실세션 데이터 축적(현재 데모 집계만; opening 3회는 데모). 관계추론은 잘못된 정규화 위에
쌓으면 되돌릴 게 많음 → 실데이터 확인 후 진입.
