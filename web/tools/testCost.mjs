...
const out = {
  ...
  environment: {
    /**
     * **판정하는 값**(2026-08-19 12차 후속 — #49의 형태): 쓰기가 실제로 막히는 것은
     * **비특권 가용 블록**(`bavail`)이 0이 될 때다. 이 컨테이너는 세션 할당량이 걸려 있어
     * `bfree`와 한 자릿수 다르다 — 실측 **bfree 244.5G ↔ bavail 29.6G**(= `df` Avail 30G).
     */
    disk_avail_bytes: (() => { try { const s = statfsSync(ROOT); return s.bavail * s.bsize; }
                               catch { return null; } })(),
    /**
     * ⚠ **연혁 필드다 — 이 값으로 판정하지 않는다.** 9차부터 적어 온 `bfree`이고
     * **세션이 못 쓰는 몫까지 센다.** 9차에 맞아떨어진 것은 그때 두 양이 함께 0이었기
     * 때문이다 — `bavail`이 0인데 이 값이 244G를 내는 상태가 성립한다(그때 이 필드는
     * 자기가 잡으려던 실패를 "여유 충분"이라 말한다).
     */
    ...
    disk_free_note: "⚠⚠ **여유가 0이면 아래 시간은 전부 무의미하다.** 9차에 전량 실행이 "
      ...
      + "⚠⚠ **읽는 필드는 `disk_avail_bytes`다**(2026-08-19 정정 · #45·#49): `disk_free_bytes`는 "
      + "`bfree`라 세션 할당량 밖까지 세고, 실측에서 둘이 **244.5G ↔ 29.6G**로 갈렸다. "
      + "⚠ **그리고 이 블록은 이 하네스가 마지막으로 돈 시각의 스냅샷이다** — 느려졌다는 "
      + "보고를 받은 **그 시점**의 값은 `df`(또는 이 하네스 재실행)로 읽는다.",
  },
