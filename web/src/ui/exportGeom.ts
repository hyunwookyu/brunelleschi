...
import type { DocState } from "./doc.js";

/**
 * **내보낼 선 하나.** L-D.2에서 `Stroke`(옛 자료구조) 대신 이 최소 모양으로 받는다 —
 * L의 문서는 `SStroke`이고 3D는 두 점(`seg3d`)이다(§1.1 프리핸드 미룸).
 * 옛 `Stroke`도 구조적으로 이것을 만족하므로 두 경로가 같은 함수를 쓴다(#17).
 */
export interface ExportLine {
  ...
  axis?: number | string;
  color?: string;
  /** 어느 뷰에서 그렸는가(§9.2). 파일에 주석·이름으로 남긴다 — 되짚을 수 있어야 한다. */
  view?: string;
}
 ...
 */
export function toObj(strokes: ExportLine[], opts: ExportOpts = {}): ExportResult<string> {
  const out: string[] = [
  ...
  for (const s of strokes) {
    ...
    out.push(`# axis=${s.axis}${s.view ? ` view=${s.view}` : ""}`
             + `${opts.colorOf ? ` color=${opts.colorOf(s)}` : ""}`);
 ...
 */
export function toGltf(strokes: ExportLine[], opts: ExportOpts = {}): ExportResult<object> {
  ...
  use.forEach((s, i) => {
    ...
    const nm = s.view ? `${s.id}@${s.view}` : s.id;    // 뷰를 이름에 남긴다(§9.2)
    meshes.push({ name: nm, primitives: [{ attributes: { POSITION: i }, mode: 3, material: mat }] });
    nodes.push({ mesh: i, name: nm });
  });
