...
export interface SStroke {
  ...
  /**
   * **어느 보조 소실점의 방향으로 놓였는가**(2026-08-19 15차 항목 7 · D-L106).
   *
   * ⚠ **축이 아니다** — `axis`는 그대로 `free`다(위계: 보조는 축을 배정하지 않고 카메라에
   * 기여하지 않는다). 이 필드는 **왜 그 방향인지**의 기록이고, 화면 안내와 진단이 읽는다.
   * 보조가 지워지면 이 id는 남지만 그 획의 기하는 안 바뀐다(이미 3D에 있다).
   */
  auxId?: string | null;
  /**
  ...
  /**
   * 끝점 판. **연쇄(`refAnchorOf`)가 이것을 앵커로 되푼다**(10차 항목 1 · 4-3 — #18 닫힘).
   * ⚠ D-L81의 "확정 전에만"은 넓어졌다 — **확정 뒤 대기 획 사이의 2D 연결도 같은 필드에
   * 적는다**(placeStroke의 대기 분기). 연결은 그리는 시점의 사실이고 시점이 언제든 같다.
   */
