...
export const RULE_TOL = {
   ...
   * 0 개입 팔) 배정이 같고 relative 배정도 0이다. 최상위 꼬리(Δ20.8·16.6)의 실제 기전은
   * **지표의 근접 퇴화**다: 그 획들은 소실점을 만든 선들이라 misfit이 구성상 0인데,
   * 극단 카메라에서 소실점이 시작점 27px 거리라 **원시 현 기준 방향각이 무의미**해진다
   * (지렛대가 획 안 — real_ink의 `near_vp_degenerate` 표식·`vp_dir_err_deg_stable`이 가른다).
   * 즉 두 표본의 차이는 카메라 품질(소실점 근접)로 설명되고(지시 4-c), ②(무앵커 무스냅)의
   * 몫은 정상 카메라의 사거리 밖 겨냥(Δ7°~12.8°)이다 — 사거리는 같은 원장의
   * `axis_snap_repair_sweep`이 든다. "Δ0.0~1.7"은 애초에 선택 인용이었다.
 ...
 * 화각 > 120°   f² > 0이지만 **쓸 수 없는 시점** → **거부**(2026-08-18 12차 지시 1-c)
 * d < 0.50·W    화각이 90°를 크게 넘는다(그 대역의 상한 f = d/2 < W/4) → **경고**(심함)
 * d < 0.87·W    아직 넓다 → **경고**
 * d ≥ 0.87·W    정상
 * ```
 *
 * ⚠ **경고는 확정을 막지 않는다**(지시 7-b "경고하고 확정하되 표시"). 막는 것은 둘이다 —
 * **f² ≤ 0**(서 있을 수 없는 카메라)과 **화각 상한 초과**(12차 지시 1-c: 물리적으로
 * 가능해도 쓸 수 없는 구간이 있다). 상한이 필요해진 실측: 실획 둘째 표본(보고)에서
 * 소실점 간격 179px(W = 1180)가 f = 87.2 · **화각 163°**로 **확정을 지나갔다** —
 * "그리드와 선이 완전히 흐트러진다"가 그 상태다. 넓지만 쓸 수 있는 화각(90~120°)은
 * 종전대로 경고 후 확정한다 — 이 도구는 **공간과 투시를 이해하는 사용자를 전제한다**(CLAUDE.md).
 *
...
export const FOV_GATE = {
  ...
  ok_f_ratio: 0.87,
  /**
   * **화각이 이 값(도)을 넘으면 거부한다**(2026-08-18 12차 지시 1-c). f² > 0이어서 카메라가
   * **존재는 하지만**, 화각 120° 위는 사람이 서서 볼 수 없는 시점이라 격자·배치가 전부
   * 흐트러진다 — 실획 둘째 표본(보고 f/W 0.074 · 화각 163°)이 이 문 없이 확정을 지나갔다.
   * `2·atan((W/2)/f) = 120°` ⟺ `f/W = 0.5/tan60° ≈ 0.2887` — severe(90°) 대역의 안쪽 절반이
   * 거부로 바뀐다. 값 120°는 지시문이 준 상한이고 스윕으로 고른 것이 아니다(#12·#26 —
   * 사전 등록이며, 조정하려면 사유를 적는다).
   */
  reject_fov_deg: 120 as number,
} as const;
...
export function fovGate(
  ...
  // **화각 상한**(12차 지시 1-c) — f² > 0이어도 이 위는 쓸 수 없는 시점이다. 안내는 한 줄이다
  // (지시 1-d): 어떻게 고치는가 — 소실점을 더 벌린다.
  if (fovDeg > c.reject_fov_deg) {
    return { band: "reject", d, dOverW, f, fOverW, fovDeg,
             why: `**서 있을 수 없는 시점입니다** — 화각 ${fovDeg.toFixed(0)}°가 상한 `
                + `${c.reject_fov_deg}°를 넘습니다. **소실점을 더 벌려 주세요** — `
                + `깊이선의 기울기를 줄이면 벌어집니다 ${tail}` };
  }
...
export function stepRule(
  ...
  if (kind === "screen_h") {
    ...
    // ⚠⚠ **스냅이 잡은 가로선은 묻지 않는다**(2026-08-18 12차 지시 4-a — D-L89 "붙은 것은
    // 묻지 않는다"). `hint === "screen"`은 직교 스냅이 실제로 발동했다는 뜻이고, D-L80의
    // kindFlipGuard가 **원 방향이 depth(≥8°)인 획에는 직교 스냅을 안 건다** — 그러므로
    // 이 hint는 원 방향이 화면 축 대역(≤ screen_axis_deg)인 획에만 붙는다. D-L70이 지운
    // `snapForced`와 다른 자리다: 그때는 좌표 이동이 방향 뒤집기(depth → screen)까지
    // 선언이 됐고(축 오차 10.1° → 31.6° 붕괴), 지금은 뒤집기가 스냅 단계에서 이미 막혀
    // 있어 그 기전이 못 돌아온다. 스냅이 안 걸린 잔여(수정자·토글 끔)만 종전대로 묻는다.
    if (forced !== "screen" && hint !== "screen" && finiteHorizontals(st).length > 0) {
  ...
  //    교점의 한 쪽(지평선)이 정확하므로 "두 선이 나란해져 교점이 날아가는" 실패가 없다.
  {
    ...
    // 두 수평 소실점 위에 얹힌다. **막는 것은 둘이다**: f² ≤ 0(카메라가 없다)과
    // 화각 상한 초과(`reject_fov_deg` — 존재하지만 쓸 수 없는 시점, 12차 지시 1-c).
