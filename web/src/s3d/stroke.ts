...
export interface PlaceCtx {
  ...
  /**
   * **축 번호 → 3D 방향**(카메라 좌표계, 단위벡터). 있으면 `vps`보다 **우선한다**.
   *
   * 왜 필요한가: 무한원 축(화면 평행, c=0)은 소실점이 없어 `vps[i] = null`인데
   * **방향은 정해져 있다** — 화면 가로면 `(1,0,0)`, 세로면 `(0,1,0)`이다(이론서 2.2).
   * 그것을 넘길 자리가 없어서 1점 투시의 가로·세로 획이 **못 올라가고 있었다**.
   * `vpRules.axisDirsOf`가 채우고, 규칙을 안 쓰는 옛 하네스에서는 `undefined`라 동작이 같다.
   */
  axisDirs?: (Vec3 | null)[];
...
export const PLACE_TOL = {
  ...
  span_far_end: 1,
  /**
   * **고치기의 후보 반경 배수**(S-7, D-S18 4). 자동 경로의 접합 반경 × 이 값이 사용자에게
   * 보여 줄 후보를 모으는 반경이다. 넉넉해서 생기는 위험(엉뚱한 후보)은 **사용자가 보고 거른다**.
   * _리뷰어 지적으로 여기 등록했다 — 임계인데 `constants`에 없으면 STALE이 안 잡는다._
   */
  fix_candidate_radius_mult: 3,
  /**
   * **같은 시점으로 볼 화면 이동 한계(px)**(S-7 2). 돌린 시점의 미배치 파선을 그 시점에서만
   * 그리는데, 그 판정을 3D 거리로 하면 `OrbitControls` damping이 계속 어긋나게 만든다.
   * 화면에서 이만큼 안 움직이면 같은 시점으로 본다.
   */
  same_view_px: 2,
