 ...
 * 안쪽 깊이만 채워진다.**
 *
 * ⚠ **1점 투시에만 성립한다**(계획서 §3.2 원문도 "1점에서"로 한정한다. 이론서 7.7 표:
 * 실척 직접 적용은 1점 폭·높이 / 2점 높이만 / 3점 없음). 2점에서는 두 수평축이 모두
 * 후퇴하므로 축 방향 (u−p, f)가 f에 의존한다 — 폭도 f를 따라 움직인다.
 * 리뷰어가 짚었고 **2점·3점에서는 확인하지 않았다**.
 *
