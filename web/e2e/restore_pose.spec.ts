...
test("주점이 화면 중심 밖(지평선 0.23H) — 궤도 진입 렌즈·회전 시점 배치가 유지된다", async ({ page }) => {
  // **1-R′ [1](높음) 대응.** 사람 보고의 표본이 이 급이다(지평선이 화면 위쪽 — 보고 y=157.
  ...
  led.offcenter = {
    ...r,
    note: "주점 중심 밖(지평선 0.23H = 이 캔버스 y 155)의 동작점(#12). ⚠ 사람 보고의 표본"
        + "(y=157)과 **같은 급**일 뿐이다 — 표본 캔버스 크기가 미상이라 비율 대응은 확인 불가"
        + "(3·4-R [16]). place_gap_px = 회전 "
...
test.afterAll(() => {
  ...
  led.what_this_does_not_say = [
    ...
    "confirm_reads.on_face_plane_residual ≈ 0도 **구성상 성립한다**(#5 유형 3 — 3·4-R [13]): " +
      "광선∩평면의 점은 어느 카메라의 광선이든 그 평면 위다. 카메라 판별은 같은 블록의 " +
      "on_face_kind가 한다 — 핀이 아니면 지면(snapCtx.ground) 자체가 없어 on_face가 안 나온다",
  ...
  led.gate = gate({
    registered: "⚠ 이 항목이 등록한 게이트다 — CLAUDE.md §2의 중단 조건(camera_gate)이 아니다(#41). "
      ...
      + "⚠ ②가 place_gap과 달리 게이트에 남는 근거(3·4-R [6]): 같은 숫자의 재적용이 아니라 "
      + "setPose → OrbitControls.update() → pose()의 **독립 재구성**을 지난다 — update()가 "
      + "자세를 덮어 왕복이 실제로 깨졌던 경로다(L-B.8 '뷰가 하나씩 늘어났다'의 그 자리)). "
