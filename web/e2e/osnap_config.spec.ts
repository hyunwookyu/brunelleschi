...
test("반경·종류·수직점 — 설정이 스냅을 실제로 가른다", async ({ page }) => {
  ...
  writeFileSync(resolve(OUT, "osnap_config.json"), JSON.stringify({
    ...
    gate: {
      ...
      reachability: "오라클 = 현행 구현의 실측(#35)",
      // ⚠ `r15_dist`는 0이었다(오조준점이 모서리 **위**라 on_edge dist 0 — 보장, #40②).
      // 정보가 있는 값은 **넓힌 조리개가 22px 밖 끝점에 실제로 닿은 거리**다.
      reachability_value: r.r40_dist ?? -1,
      reachability_source: "measured/r40_dist",
    },
    measured: r,
    what_this_does_not_say: [
      "15px가 **옳은 기본값이라는 것** — 합성 측정은 반대를 가리킨다(D-L56: snap.json 스윕에서 "
