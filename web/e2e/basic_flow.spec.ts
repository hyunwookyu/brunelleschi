...
test.skip("기본 흐름 — 긋고, 서고, 덧긋고, 돌리고, 이어 긋는다", async ({ page }) => {
  ...
  writeFileSync(resolve(OUT, "basic_flow.json"), JSON.stringify({
    ...
    what_this_does_not_say: [
      ...
      "⑥의 lifted 여부는 단언하지 않는다 — 복귀한 작도 시점의 배치도 앵커·각도에 따라 2D 대기가 옳을 수 있다(§9.1 — s6_continue.lifted는 기록값이지 단언이 아니다). ⚠ 14차 항목 6 계약 갱신(D-L97): ⑥은 이제 **모델링 차단 음성 대조(s6_blocked — 획 수 불변) + 「작도 시점으로」 복귀 + 복귀한 작도 시점(s6_view_state)에서의 이어 긋기**다 — 옛 계약(임의 돌린 시점에서 바로 긋는다)은 지시 6-e가 닫았다",
    ...
    gate: {
      registered: "④ 결과선이 보조선 자리를 15 이상 어둡게 하고 절대 70 미만(5-1 소실·5-7 검정의 회귀) · "
        + "①~⑥ 각 단계의 픽셀·차수·스냅 단언(⑥은 14차 D-L97 계약: model 차단 → 복귀 → 작도 시점에서 스냅 — s6_blocked·s6_view_state·s6_continue) · 콘솔 오류 0. "
