...
test("큐브 정면 탭 — 그린 상자의 입면으로 난다 (10차 항목 6)", async ({ page }) => {
  ...
  writeFileSync(resolve(OUT, "cube_frame.json"), JSON.stringify({
    ...
    what_this_does_not_say: [
      ...
      "**old_frame 팔의 값도 크기로는 정보가 없다**(리뷰어 5차 [1] · #40): 옛 배선이 세계 −z로 나는 한 dot_gaze_world_z는 항등 1이고 dot_gaze_depth는 이 픽스처에서 cos35°·cos15° 외의 값을 낼 수 없다(본 팔이 기각당한 그 상수와 같은 수다). **이 원장이 재는 것은 크기가 아니라 자리바꿈이다** — 정확히 1이 어느 슬롯에 오는가(본 팔: depth / 옛 팔: world_z)가 비행 방향의 범주 사실이고, 두 팔의 실행이 그 바꿈을 실측한다. 크기가 정보를 실으려면 요·피치를 흔든 둘째 구도에서 값이 cos(요)·cos(피치)를 따라가는지 봐야 한다(DEFERRED — 픽스처가 각도 매개변수화를 안 받는다)",
