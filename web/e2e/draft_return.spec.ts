...
test("작도 화면 복귀 — 돌린 뒤 소실점·지평선·그리기가 돌아온다 (14차 항목 6)", async ({ page }) => {
  ...
  writeFileSync(resolve(OUT, "draft_return.json"), JSON.stringify({
    ...
    what_this_does_not_say: [
      ...
      "**재탭 임계의 경계 스윕은 view_cube 팔(basic_flow)이 잰다** — 값은 view_cube.json의 cube_tol.retap_cos 필드가 든다(#47). 여기서는 '실제로 도는가'(지시 6-b)만 잰다",
      "**복귀 뒤 각도·정합의 near-zero(피치 ~1e-15·vps_y_gap 0·주점 정합 ~1e-13)는 정확 스냅의 귀결이다**(selfcheck near-zero 플래그의 원인 — §5): snapToDir가 정확한 방향으로 돌리고 픽스처 축이 정확 직교라 잔차가 부동소수 꼬리뿐이다. 판정 임계(pitch_return_max_deg 0.1·coherence_max_px 1)는 그 보장이 깨졌을 때(스냅이 다른 프레임으로 가거나 표시가 다른 출처를 쓸 때)를 가르는 배선 여유다 — 정확도 측정이 아니다",
