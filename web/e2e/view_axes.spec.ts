...
test("어느 시점에서든 축이 보이고 그릴 수 있다 (D-L101·D-L102)", async ({ page }) => {
  ...
  const led = {
    ...
    how: {
      ...
      horizon: "지평선은 **배선을 잰다**(#5) — `horizon_gap_px`(두 수평 소실점까지의 "
        + "점-선 거리, px #49)는 선을 그 두 점으로 만든 이상 0이 **보장**이라 옳음을 "
        + "못 가른다. ⛔ 초판은 이 자리에 «옳음을 잰다»라고 적었다(2차 리뷰어 [5]). "
        + "판정이 드는 값은 `old_flat_gap_px_rotated`(옛 화면 가로선이 둘째 소실점에서 "
        + "벗어난 px — 핀 자세 제외, #30의 양성 채널)다.",
    ...
    what_this_does_not_say: [
      ...
      + "판정은 `old_flat_gap_px_rotated`(옛 판이 벗어난 크기 — 핀 제외)가 든다.",
      ...
      "`finite_axes`는 **축 셋 중 유한한 소실점을 가진 축의 수**다(0..3) — 수평 소실점 "
      + "개수가 아니다(그 양은 `draft_return.json`이 `vps_h_finite`로 따로 적는다, "
      + "2차 리뷰어 [7]). 자세 다섯에서는 셋이 다 유한이라 `infinite` 표식이 안 나온다(#35 — 도달 불가를 "
      + "안전으로 안 읽는다) — 이것은 **이 다섯 자세의 성질이지 일반 성질이 아니다**: 같은 "
      + "세션의 `draft_return.json`은 피치 −39.86°의 돌린 3점 자세에서 "
      + "`rotated_three_own_horizon.finite_axes` **2**를 낸다(같은 정의). 그 갈래는 "
      + "`infinite_arm`(2점 규칙을 넣어 수직축을 무한원으로 "
      + "만든 팔)이 든다.",
      ...
      + "**획이 문서에 남고**(`stroke_added` 참·`blocked_now` 거짓 — 이 원장은 **스냅을 "
      + "안 쟀다**, 2차 리뷰어 [3]) 먼 소실점이 무엇을 "
      + "망가뜨리는지 재는 팔이 없다 — 그래서 안 좁혔다. 판정은 `DEFERRED`.",
      "그릴 수 있는가는 **획이 문서에 남는가**로만 본다 — 그 획이 3D에 **옳게** 놓이는가는 "
      + "여기서 안 잰다. ⛔ 초판은 그 몫을 `dir_state`·`confirm_link`로 넘겼는데 **그 두 "
      + "원장에 돌린(비핀) 자세의 행이 0건**이라 위임할 자리가 없었다(2차 리뷰어 [2]). "
      + "**돌린 자세에서 그은 획의 배치 옳음을 재는 원장은 없다** — 참값 대조를 앱에서 "
      + "안 하기 때문이다(`stage_browser.json`의 `l_b8` ws가 같은 말을 한다). 판정은 `DEFERRED`.",
    ],
    selfcheck_notes: {
    ...
    gate: {
      ...
      /**
       * **픽스처가 정한 상수가 아닌가**(#46 — 2차 리뷰어 PITFALLS 대조). `false`다:
       * 이 값은 자세의 회전각에 따라 움직이고(핀 0 · 요·배면·윗면이 서로 다르다),
       * 코드를 틀리게 하면(지평선을 다시 화면 가로선으로 두면) 0으로 무너진다.
       */
      reachability_value_fixture_determined: false,
      result: {} as Record<string, unknown>,
    },
    pitfall_citations: [5, 12, 17, 21, 30, 34, 35, 40, 46, 47, 49],
    errors,
