...
test.describe("dpr 2", () => {
  ...
  test("같은 손짓이 같은 각을 돈다 — 제스처 좌표는 css 픽셀이다(#21)", async ({ page }) => {
    ...
    led.dpr2 = {
      ...
      rotate: { azimuth_delta: az, dpr1_azimuth_delta: one, ratio: az / one!,
                ...
                az_tol_note: "판정은 |az−dpr1| < 1e-6rad(6차 항목 1에서 개정, #28 — 옛 정밀도 12"
                  ...
                  + "차는 감쇠 적분의 프레임 타이밍을 타 실행마다 1e-10대에서 흔들린다 — 이 "
                  + "원장의 azimuth_delta 두 필드가 이 실행의 값이다. 6차 컨테이너(chromium-1194)"
                  + "에서 연속 4회 5e-13을 넘어 실패했고 변경 전 코드에서도 재현됐다(코드 결함 "
                  + "아님). 실행마다 흔들리는 값이므로 5차의 '간헐·단독 통과' 기록과 정합한다)",
    ...
    // (Δ≈0.5rad)이지 1e-10대 잡음이 아니다. 두 팔의 차는 감쇠 적분의 프레임 타이밍을 타
    // 실행마다 흔들리고(6차 컨테이너에서 연속 4회 1e-10대로 실패 — 변경 전 코드에서도 재현),
    // 그래서 5차의 "간헐·단독 통과"로도 나타났다. 1e-6rad은 오배선과 다섯 자리 여유로 갈린다.
