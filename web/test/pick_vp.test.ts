...
//   #8   p90·max를 함께 낸다
//   #9   pick 실패(호스트 선이 조리개 밖) 실행이 섞인다 — **picked 층을 갈라 센다**
 ...
 * 찍기가 실패해도(호스트 선이 조리개 OSNAP_RADIUS_PX 밖 — D-L85로 8px) 이어 긋는다 — 앱에서도 그렇다(#11).
 */
    ...
    const out = {
      ...
      how: {
        ...
        pick_point: "참 소실점 위치 + 보조선 방향을 따라 ±off px. 찍기의 확정점은 그 점의 "
          ...
          + `결과에 섞인다 — 그것이 이 경로의 실제 오차 전달이다. 수직 성분이 조리개(${OSNAP_RADIUS_PX}px — D-L85)를 넘으면 `
