  ...
  it("§4.5 미리보기 ↔ 확정 — AS-3(유일한 중단 조건, D-S6)", () => {
    ...
    const report = {
      ...
      gate: gate({
        ...
        // **오라클 팔이 없다**(#40) — 그 사실을 산문에 묻지 않고 필드로 적는다.
        reachability_absent: "**오라클 팔이 없다.** 이 게이트에는 '참값을 넣으면 무엇이 나오는가' "
          + "팔이 없고, 그래서 #35대로 **기준을 못 넘은 것이 신호의 성질인지 기준의 성질인지 "
          + "갈리지 않는다**. 가장 가까운 양성 채널은 `unanchored_threshold`가 끝점 겨냥 오차 "
          + "2%·4%에서 실제로 발동한다는 것이고(그 값은 post_hoc이라 판정 근거로 안 쓴다), "
          + "그것이 하는 일은 **발동 가능한 관측 조합이 존재함**을 보이는 것 하나다. "
          + "⚠ 기준을 낮추는 근거가 아니다 — 적기만 한다.",
