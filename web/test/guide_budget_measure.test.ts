  ...
  it("측정을 원장에 남긴다", () => {
    ...
    const doc = {
      ...
      gate: gate({
        ...
        // ⚠ **초판은 588.18px(캔버스 안 최대 현)을 값으로 적었는데 그것은 기준과 다른 지표다**
        // (기준은 "예산이 **늘어난다**"이고 588.18은 **길이**이며, 게다가 다른 요구의 도달
        // **불가**를 말한다). 리뷰어가 잡았다 — **오라클 팔이 없다는 것이 사실이므로 그렇게 적는다.**
        reachability_absent: "**오라클 팔이 없다.** 이 게이트의 기준은 방향 주장('최소 예산이 "
          + "늘어난다' · 'f는 안 바뀐다')이고, 참값을 넣어 무엇이 나오는지 보는 팔이 없다. "
          + "가장 가까운 것은 **상한**이다 — 늘린 뒤 길이는 캔버스 안 현이 상한이고 그 최대가 "
          + "588.18px인데 `min_guide_ratio`가 요구하는 것은 651.2px다(`extend_ceiling`). "
          + "⚠ **그것은 이 게이트의 기준이 아니라 다른 요구의 도달 불가**이므로 값 필드에 안 적는다. "
          + "⚠ 기준을 낮추는 근거가 아니다 — 적기만 한다(#35).",
