    ...
    const out = {
      ...
      jit0_aperture: {
        note: "**jit_0 층의 조리개 정보량**(8-R″ [7], #38) — true면 그 층에서 스윕 팔 요약이 "
          ...
          + "비교 집합은 headline_jit0_no_identity의 summarize() 요약 필드 **전부**"
          + "(JSON 문자열 동일성 — 8-R‴ [5]). false는 '어딘가 다르다'다 — 어느 필드가 갈리는지는 "
          + "그 층을 그 자리에서 읽는다(현행 drawn: 발동 수·첫 짝 구성이 갈리고 중앙값·camera_ok는 "
          + "동일 — 값은 원장 필드). 동일한 사유는 안 갈랐다(추정 — jit_0은 끝점 오차 0이라 "
          + "오스냅 발동이 조리개에 안 걸리는 쪽 — 은 검증 안 됨).",
        ...jit0Aperture,
      },
      paired_headline: {
