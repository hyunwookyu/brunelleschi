    ...
    const out = {
      ...
      how: {
        ...
        ask_policy:
          ...
          + "비용이다. ⚠⚠⚠ **판은 넷이다 — 이 필드가 같은 자리에서 세 번째로 낡았다**(#34, "
          + "2026-08-18 8차 2차 지시 8-2R′ [9]. 앞의 '두 번째'를 '세 번째'로 읽는다): "
          + "① 물음 오라클 `git show d724ac0:` ② 무물음 선언 `git show b52bc7c:` "
          + "③ 강제 없음(D-L70) `git show d85dca8:` ④ **P1 잠김을 연 판 = 현행(이 파일)** "
          + "`d233ec4`. ⚠ **한 원장 안에 같은 사실을 적는 자리가 둘이다** — 이 필드와 "
          + "`what_this_does_not_say[2]`. 하나를 고치면 다른 하나를 함께 본다(#34).",
      },
      headline: {
