...
//
// ⛔⛔ **은퇴 — 재목적 완료**(2026-08-17 5차 이월-3): 이 하네스의 조작 변수(지평선 높이)는
// D-L59로 죽었다(지평선은 결과다 — 소실점 확정에 안 쓰인다). 세 팔이 동일값을 내므로 이
// 원장은 **역사 기록**이고, 찍기 경로(4차 지시 4-b)의 측정은 **`pick_vp.test.ts` →
// `stage0/out/pick_vp.json`이 잇는다.** 파일을 지우지 않는 이유: 인용 대상 유지(D-L54의
// 동결과 같은 자리 — 지우면 해시 대조가 '없는 원장 인용'으로 깨진다) + 재발 시 되짚을 근거.
//
 ...
 * ②·③도 팔이 같아 가를 것이 없다. 등록문은 역사 기록으로 남긴다 — **재목적은 완료됐다**
 * (5차 이월-3): 찍기 경로 측정은 `pick_vp.json`이 잇는다. */
const REGISTERED =
...
describe("지평선 = 피치 — 사용자가 끌면 3점이 초기부터 서는가 (D-L45)", () => {
  it("중앙·참·위약 + 허용 오차 스윕을 같은 픽스처에서 함께 낸다", () => {
    ...
    const out = {
      what: "⛔ **은퇴 — 역사 기록**(5차 이월-3): 조작 변수(지평선 높이)가 D-L59로 죽어 세 팔이 "
        + "동일값을 낸다. 찍기 경로 측정은 **pick_vp.json**이 잇는다. 원래 재던 것: 지평선을 "
        + "사용자가 끄는 것(= 피치)의 전제 — 지평선이 맞으면 초기 스케치에서 차수가 3이 되는가.",
