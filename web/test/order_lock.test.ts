...
const REGISTERED =
  ...
  + "`beyond_cut_0_5`가 안 는다. ⚠⚠ **판정은 result의 `pass_two_sided_vs_guard_off`를 그 "
  + "자리에서 읽는다**(산문에 판정을 박지 않는다 — 12차 리뷰어 [2]가 산문 '실패한다'와 "
  + "필드 true의 어긋남을 잡았다, #41의 자리). 연혁(⚠ 팔 이름 병기 — 2차 [6], #47): "
  + "조항 판정은 `beyond_cut_0_5_fixed ≤ _guard_off`의 **팔 사이 대비**이고, 그 왼쪽 항 "
  + "`_fixed`가 9차 1923 → 1965, 10차 D-L85 뒤 2115로 움직이는 동안 판정 필드는 계속 "
  + "false였다 — **12차(화각 상한 등록 재실행)에서 처음 true다**(현행 두 항은 result의 "
  + "`beyond_cut_0_5_fixed`·`_guard_off`를 그 자리에서 읽는다). "
  + "⚠ 귀속은 못 적는다: 커밋 사이 변화라 대조 팔이 없다(위 ③의 경고 그대로). "
    ...
    const out = {
      ...
      gate: gate({
        ...
        result: {
          ...
          guard_off_note: "④ **가드의 두 방향**(`fixed` ↔ `guard_off`). ⚠⚠ **판정은 이 블록의 "
            + "`pass_two_sided_vs_guard_off`를 그 자리에서 읽는다** — 산문에 판정을 박았다가 "
            + "값과 어긋났다(12차 리뷰어 [2] — 산문 '실패한다' ↔ 필드 true, #41·#47). "
            + "정확·파국의 현행 값은 `within_cut_0_2_*`·`beyond_cut_0_5_*`를 읽는다"
            + "(연혁 — ⚠ 팔 이름 병기(2차 [6], #47): `within_cut_0_2_fixed` 9차 46→52 · "
            + "10차 68→85 / `beyond_cut_0_5_fixed` 9차 1923→1965 · 10차 2115 / "
            + "`beyond_cut_0_5_guard_off` 10차 2110 — 전부 커밋 사이의 한 팔 역사이고, 조항 "
            + "판정 자체는 fixed↔guard_off **팔 사이 대비**다. 12차 화각 상한 재실행에서 "
            + "**처음 통과** — 귀속은 커밋 사이 비교라 못 적는다). "
            + "**통과가 나도 기준을 그대로 둔다**(#26·#35) — 과거의 실패 연혁이 D-L80 대가의 "
            + "기록이고, 그 결정문이 같은 대가를 적고 있다(#15·#16).",
        },
