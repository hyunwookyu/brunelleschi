...
const REGISTERED =
  ...
  + "`beyond_cut_0_5`가 안 는다. ⚠⚠ **이 조항은 지금 실패한다**(`beyond` 1923 → 1965). "
    ...
    const out = {
      ...
      gate: gate({
        ...
        result: {
          ...
          guard_off_note: "④ **가드의 두 방향**(`fixed` ↔ `guard_off`). ⚠⚠ **이 조항은 실패한다** — "
            + "정확(`within_cut_0_2`)은 46 → 52로 늘었지만 **파국(`beyond_cut_0_5`)이 1923 → 1965로 "
            + "늘었다**. **실패를 적고 기준을 안 낮춘다**(#26·#35). D-L80을 되돌리는 근거가 아니라 "
      ...
      factor_split: {
        ...
        note: "**중앙값으로 읽으면 방향이 갈리지 않는다**(8차 항목 1 이후 재측정): "
            ...
            + "⚠ 특히 **46은 이제 `guard_off`의 값**이라 옛 문장을 그대로 두면 "
            + "`bypass`·`fixed`의 수로 오독된다.",
