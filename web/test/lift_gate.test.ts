  ...
  it("초기 올리기 게이트를 원장에 낸다", () => {
    ...
    const doc = {
      ...
      gate: gate({
        ...
        // ⚠ **초판은 조용히 틀림 0.2552를 적었는데 그것은 등록 지표가 아니다**(등록은 확정·무변화·
        // 전부 놓임이다). 등록 지표에 맞는 오라클은 **배치율이고 그 값이 정확히 1.000**이다 —
        // 그런데 그 1은 **분모가 최대 연결 성분**이라 나오는 수라(#27) 측정이 아니라 **구성의 귀결**이다.
        // 즉 **정보량 있는 오라클이 없다**. 그것이 사실이므로 그렇게 적는다(#40).
        reachability_absent: "**정보량 있는 오라클이 없다.** 등록 지표(연결된 획이 전부 놓이는가)의 "
          + "오라클 팔은 `by_jitter/jitter_0/true_camera_oracle/placement/rate` = **717/717 = 1.000**인데, "
          + "그 분모가 **최대 연결 성분**이라 1이 나오는 것이 구성의 귀결이다(#27이 이 원장에 대해 "
          + "이미 그렇게 적었다). **항등을 도달 가능성으로 쓰지 않는다**(#40 ②). "
          + "⚠ 같은 블록 산문의 0.0791은 **baseline 3pt 팔**(34/430)이고 이 원장의 5구도 jitter_0 팔은 "
          + "183/717 = 0.2552다 — **다른 부분집합이다**(#9). 어느 쪽도 등록 지표가 아니다.",
      ...
      compare_prior: {
        ...
        source: "docs/archive/segment_plan.md 머리말 · segment_gate.json@509615a1",
      },
      constants: constantsSnapshot(),
      metric_defs: metricsSnapshot(),
    };
