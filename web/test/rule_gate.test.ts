    ...
    const doc = {
      ...
      gate: gate({
        ...
        reachability:
          ...
          + "초판은 여기서 '신호의 성질이고 기준의 성질이 아니다'라고 단정했는데 **그 근거가 "
          + "항등이었다** — 대역 팔이 통과선 안이라는 것은 1.0 ≤ 1.1 · 1.0 ≤ 2.1이고, 대역은 "
          + "그 팔 자신이다(리뷰어 [1]이 잡았다. L-C.3의 자기 게이지와 같은 유형). "
          + "**비오라클 팔이 1.1×·2.1× 안으로 들어올 수 있다는 것은 안 보였다** — 셋 중 최선인 "
          + "`rule_grouped`가 형태에서 통과선의 3.4배, 조용히 틀림에서 2.1배 밖이다. "
          + "그 상태를 적는 것이 #35가 요구하는 전부이고, **기준은 그대로 둔다.**",
        // **세 팔 중 형태가 가장 좋은 팔**을 고른다 — "얼마나 잘할 수 있는가"라 부르면서 같은
        // 표에 더 좋은 팔을 두지 않는다(리뷰 [11]). 두 지표를 다 적는다(#28: 못 고르면 둘 다).
        reachability_value: [
        ...
        population_caveat:
          ...
          + "**획을 거의 못 놓기 때문**(P1에 갇히면 연결 성분이 잘게 부서진다)이고, 그래서 "
          + "'놓인 획 중 틀린 비율'의 합산은 **주로 1점·2점 구도의 수**다. "
          + "⇒ **표제 배수(형태 9.4배)를 경로 전체의 대표값으로 읽지 않는다.** "
          + "판정 자체는 안 바뀐다 — `result.by_composition`의 **다섯 구도 전부**가 조용히 틀림에서 "
          + "떨어지고 형태는 넷이 떨어진다(1점 형태 한 칸만 선 안이고, 그 칸의 대역은 아래 "
          + "`one_point_band_caveat`대로 부풀려져 있다). "
        ...
        mechanism:
          ...
          + "3점 구도에서도 P1이 40~47/60이다(`by_composition`). P1은 **불가역**이므로(D-L53) "
          + "한 획이 그림 전체를 1점에 가둔다. "
          + "⚠⚠ **잡음의 산물이 아니다**: 가장 깨끗한 층(`by_jitter.jit_0`)에서 판정 팔의 차수는 "
          + "**P1 60/60**이고 조용히 틀림이 146/184다. 끝점 지터 0에서도 **전부** 갇힌다. "
          + "⚠ **P1 잠김 자체는 이번의 발견이 아니다**(리뷰어 [9]) — `rule_camera.json`이 전 팔에 "
          + "`order_median: 1`을 이미 적고 있었고 AS-L24·AS-L25가 이 경로의 축 오차 열화를 반증으로 "
          + "등재해 뒀다. **이번에 새로 보인 것은 둘이다**: (가) 그 잠김의 **배치·형태 귀결**의 크기, "
          + "(나) 화면 직교 스냅이 P1 가드를 우회한다는 **지목**. "
          + "\n\n"
          + "**반사실(`rule_drawn_no_ortho_force`)이 실제로 보인 것**: 화면 직교 스냅의 "
          + "`forced=screen`이 `stepRule`의 P1 가드(`forced !== screen`일 때만 묻는다)를 우회한다 "
          + "— 그 우회만 끄면 P1이 243 → 181로 줄고 P3가 33 → 75로 는다. "
          + "**그런데 옳게 놓인 획은 하나도 안 는다.** 비율이 아니라 **절대수**로 읽는다"
          + "(`inside_cut` — #14·#16, 리뷰어 [3]이 이 읽기를 짚었다): "
          + "절단 0.2 안 **85 → 85** · 절단 0.1 안 **44 → 44**. 놓인 획은 932 → 1175로 **243개 늘었고 "
          + "그 243개가 전부 절단 밖**이다. **시드 여섯 전부에서 같다**(14/14 · 20/20 · 15/15 · "
          + "15/15 · 10/10 · 11/11 — `by_seed`) — 시드 폭 안의 흔들림이 아니라 **정확한 불변**이다. "
          + "⇒ 그 우회를 없애면 **병목이 옮겨갈 뿐 없어지지 않는다**(PITFALLS #16의 형태 그대로: "
          + "G-A.1에서 끝점 의존이 사라지고 배치율이 1.7배가 됐는데 조용히 틀린 배치는 늘었던 그 자리). "
          + "초판이 이것을 '기여이지 지배항이 아니다'라고 적었는데 **더 정확한 진술은 '기여가 아니라 "
          + "재분배다'** — 배치 정확도에는 기여가 **0**이다(축 오차·차수 분포에는 있다). "
          + "\n\n"
          + "⚠ 축 오차는 **1pt를 뺀 층에서 읽는다**(`headline_no_identity`, 리뷰어 [8]): "
          + "판정 팔 **35** → 반사실 **31.61**이다. 표제 표의 35 → 24.87은 1점 구도의 **구성 항등 0**이 "
          + "섞인 값이라 감소폭이 부풀려져 있었다(그 구도는 소실점 하나가 주점 자리라 축 방향이 "
          + "f와 무관하게 광축이 되고 **어떤 오차를 넣어도 0**이다). "
          + "그리고 이 분포는 연속이 아니라 **구도의 yaw에 뭉친 범주**이므로(35 / 37.70 / 40.66 / 31.61) "
