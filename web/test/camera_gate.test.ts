  ...
  it("축 방향 오차 → 형태 오차 곡선을 원장에 낸다", () => {
    ...
    const doc = {
      ...
      gate: gate({
        ...
        // ⚠⚠ **사정거리**(2026-08-17 8차 지시 b — PITFALLS #43). 아래 `scope_note`가 그것이다.
      ...
      // ⚠⚠ **사정거리 — 2026-08-17 8차 지시 b** (PITFALLS #43)
      scope_note: {
        what_this_gate_touches:
          "**소실점 → `recoverCamera` → `liftAll` → 형태 오차.** 소실점은 이 하네스가 **직접 준다**"
          + "(참값 또는 각도 섭동판). 즉 재는 것은 **카메라 수학과 배치**이고 그 구간뿐이다.",
        what_it_does_not_touch:
          "**확정 규칙 경로 전체가 사정거리 밖이다** — 획을 받아 2D 판정(`resolve2dCore`)을 "
          + "거쳐 `stepRule`이 소실점을 세우는 구간. 그쪽을 고쳐도 이 원장은 **한 자리도 안 "
          + "움직인다**. 화면 뒤(뷰 복원·궤도 렌즈·3D 오스냅)도 마찬가지로 밖이다.",
        why_it_is_written_here:
          "**원장이 안 움직인 것은 안전의 증거가 아니다.** 8차 지시 b가 이것을 지적했다: "
          + "확정 규칙을 바꾼 변경이 이 원장을 안 움직였고 그 부동이 '안전하다'로 읽혔다. "
          + "그 읽기는 틀렸다 — 이 하네스는 그 구간을 **애초에 안 지난다**. "
          + "PITFALLS #40 계열(검사를 만들면서 그 검사를 무력화한 것)의 사정거리 판이고 "
          + "**#43**으로 등재했다. 부동을 근거로 인용하려면 **그 변경이 이 경로 위에 있는지** "
          + "먼저 확인한다.",
        the_other_gate:
          "확정 규칙 경로는 `stage0/out/rule_gate.json`이 잰다(8차 지시 a로 신설). "
          + "⚠ **두 게이트를 합산해 하나의 '통과'로 읽지 않는다**(#41) — 대역도 경로도 다르다. "
          + "⚠ 그 원장은 **첫 실행에서 실패했다**: 앱이 실제로 쓰는 경로(획 → 확정 규칙 → 카메라)의 "
          + "**형태 오차 중앙이 대역의 9.4배**다(지표를 명시한다 — 조용히 틀림 기준으로는 2.4배이고, "
          + "두 배수 다 모집단 혼합에 민감하다: `rule_gate.json`의 `gate.population_caveat`). "
          + "이 원장의 `deg_0.25` 통과는 **그 경로의 통과가 아니다** — 여기서 통과한 것은 "
          + "'사람이 소실점을 0.5°로 맞췄을 때의 카메라 수학'이다.",
      },
