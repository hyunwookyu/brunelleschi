...
export type Gate = {
  ...
  result?: unknown;
  /**
   * **실패했으면 기전을 적는다**(2026-08-17 8차). 실패한 게이트가 수치만 남기면 다음 세션이
   * "기준이 빡빡한가 신호가 나쁜가"를 다시 처음부터 판다. 반사실 팔로 원인을 좁혔으면
   * **좁힌 만큼과 안 좁혀진 만큼을 함께** 적는다 — 기여를 지배항으로 적지 않는다(#39).
   */
  mechanism?: string;
  /**
   * **판정의 크기가 모집단 혼합에 민감하면 그 사실을 적는다**(2026-08-17 8차 리뷰어 [4]).
   * 통과/실패가 안 바뀌어도 **배수의 크기**는 바뀔 수 있고, 그 배수가 다른 문서로 인용된다.
   */
