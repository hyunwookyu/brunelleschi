 ...
 *
 * ⚙️ **그 전제를 기계가 지킨다**(2026-08-19 17차 1차 리뷰어 [10]): 초판은 «측정에 들어가면
 * 그때 옮긴다»를 **사람이 기억하는 규약**으로 뒀는데, 같은 저장소가 *"사람이 지켜야 하는
 * 규약은 샌다(#19·#38)"*고 반복해서 적는다. 그래서 `selfcheck.scan_unhashed_thresholds`가
 * **이 목록의 이름이 `stage0/out`의 원장에 나타나면 플래그한다** — 나타났다는 것은 그 원장이
 * 이 값에 의존할 수 있다는 뜻이고, 그러면 STALE 밖에서 낡는다. 잡히면 할 일은 둘 중 하나다:
