...
// **기하 계층뿐**임이 보장된다. 옛 값은 `fix_load.json@2fd74e1c`에서 온다(재실행 없이 인용).
//
...
/** 옛 값 — `fix_load.json@2fd74e1c`(사용자 모형 `nearest`). 재실행 없이 인용한다. */
const OLD = {
    ...
    const report = {
      ...
      gate: gate({
        ...
        reachability: "**옛 값이 같은 판정 계층에서 나왔다** — `classifyStroke`를 같은 설정으로 쓰므로 "
          ...
          + "(`fix_load.json@2fd74e1c`, 재실행 없이 인용). ⚠ **다른 하네스의 값이므로 대역으로 "
          + "쓰지 않는다**(#27) — 자릿수 대조다.",
      ...
      condition: {
        ...
        old_values_from: "fix_load.json@2fd74e1c (사용자 모형 nearest). 재실행 없이 인용한다.",
