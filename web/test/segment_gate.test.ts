...
// **기하 계층뿐**임이 보장된다. 옛 값은 `fix_load.json@1671e540`에서 온다(재실행 없이 인용).
//
...
/** 옛 값 — `fix_load.json@1671e540`(사용자 모형 `nearest`). 재실행 없이 인용한다. */
const OLD = {
  ...
  it("같은 합성 조건에서 재측정하고 옛 수치와 대조한다", () => {
    ...
    const report = {
      ...
      condition: {
        ...
        old_values_from: "fix_load.json@1671e540 (사용자 모형 nearest). 재실행 없이 인용한다.",
