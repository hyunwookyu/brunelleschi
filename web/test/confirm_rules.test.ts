...
function fovSection() {
  ...
  const sweep = [0.2, 0.4, 0.49, 0.5, 0.6, 0.86, 0.87, 1.0, 1.5, 2.0, 3.0].map(rf => {
  ...
  return {
    ...
    thresholds: FOV_GATE,
    covered: sweep.length + 2,
    sweep,
    /** `gate.reachability_value`가 가리키는 자리 — severe · warn · ok 순(#40 값 대조). */
    ...
    real_ink_reported: {
      source: "6차 지시문 보고 — **파일이 sessions/에 없다**(real_ink는 awaiting_samples)",
    ...
    gate: gate({
      registered: "f² ≤ 0이면 **거부**한다. **f/W**(시거리 ÷ 화면 폭) < 0.87이면 경고하되 "
                + "**확정은 막지 않는다**(지시 7-b: 경고하고 확정하되 표시). 화면 밖 소실점은 "
                + "**정상**이므로 거르지 않는다. ⚠ **초판은 이 문장을 `d/W`로 적었고 6-R이 대역을 "
                + "f/W로 옮겼는데 등록문이 안 따라왔다** — 7차 항목 4에서 정정했다(#24: 단위가 "
                + "다르면 같은 임계가 아니다. d에 걸면 실효 임계가 정확히 두 배 느슨해진다). "
                + "**[7차 항목 4 추가]** 그 게이트가 **확정 경로에 걸려 있는지**는 "
                + "`vp_rules.test.ts`의 배선 팔 셋이 잠근다(거부 · 양성 채널 · 2점 동시 경로).",
      reachability: "대역이 실제로 갈리는가 — **쓸기 11행은 severe·warn·ok 셋을 내고 "
                  + "reject는 `counter_example`이 낸다**(쓸기는 f² > 0 격자라 reject가 안 나온다). "
                  + "한 대역만 나오면 판정이 아니다(#30). `reachability_value`는 "
                  + "`band_counts_ordered` = **[severe, warn, ok]** 순이다",
  ...
  it("네 절을 재고 원장에 남긴다", () => {
    ...
    writeFileSync(resolve(OUT, "confirm_rules.json"), JSON.stringify({
      ...
      why: (
        ...
        + "화각 게이트 부재 · 다른 축의 대각선 둘이 1점으로 굳음. **파일 자체는 저장소에 없다** — "
        + "`sessions/`가 비어 있고 `real_ink`는 `awaiting_samples`다. 그래서 여기서는 보고된 "
        + "증상을 **코드에서** 재현·판정하고, 수치가 필요한 자리는 합성 구도로 잰다."
      ),
      what_this_does_not_say: [
        "**실획이 아니다.** `sessions/`가 비어 있어 `real_ink`는 `awaiting_samples`다 — "
        + "`fov_gate.real_ink_reported`의 입력은 **지시문이 보고한 수**이고 파일 측정이 아니다.",
