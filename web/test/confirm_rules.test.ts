...
function fovSection() {
   ...
   * 슬롯 좌표(901.95, −231.04 · 지평선 157.03)가 이 보고와 반올림 차이로 일치한다 —
   * "6차 보고 파일"이 곧 그 표본이었다. 실좌표 측정은 `real_ink.k_metrics.per_doc`의
   * `camera_assumed_principal_center`가 낸다(#17 — 같은 f² 식).
   */
  const reportedW = 1180;
  const reported = fovGate([902, 157], [-231, 157], [reportedW, 668]);
  /**
   * **실획 12차 두 표본**(12차 지시 1-f — 회귀 팔). ⚠ **파일이 도착했다**(2026-08-19 —
   * `sessions/`의 15-16-18·15-18-25). 실좌표 측정은 `real_ink.k_metrics.per_doc`가 내고,
   * 아래 역산 재구성은 **보고값 기준 회귀 팔**로 남는다(파일 좌표와의 대조는 real_ink 쪽).
   *
  ...
  return {
    ...
    real_ink_reported: {
      source: "6차 지시문 보고 — ⚠ 2026-08-19 정정: **그 파일이 8-17 표본이었다** — sessions/의"
            + " brunelleschi-2026-08-17T11-41-36 슬롯 좌표(901.95, −231.04)가 이 보고와 반올림"
            + " 차이로 일치한다. 실좌표는 real_ink.k_metrics.per_doc이 잰다",
    ...
    real_ink_reported_12: {
      source: "12차 지시문 보고 — ⚠ 2026-08-19: **파일이 sessions/에 도착했다**. 이 블록은 "
            + "보고값 기준 회귀 팔이고 실좌표 측정은 real_ink.k_metrics.per_doc이 낸다. "
      ...
      file1: {
        ...
        note: "경고(severe) 대역 — **확정은 지나간다**(지시 1-e: 표시가 남는 것이 맞는 동작). "
            ...
            + "(리뷰어 [11] — 파일 2와 필드가 비대칭인 이유). 2026-08-19 파일 도착 — 실좌표는 "
            + "real_ink.k_metrics.per_doc의 camera_assumed_principal_center가 낸다",
      },
      file2: {
  ...
  it("네 절을 재고 원장에 남긴다", () => {
    ...
    writeFileSync(resolve(OUT, "confirm_rules.json"), JSON.stringify({
      ...
      why: (
        ...
        + "화각 게이트 부재 · 다른 축의 대각선 둘이 1점으로 굳음. ⚠ **2026-08-19 정정: 표본 "
        + "파일 셋이 전부 sessions/에 있다**(6차 보고 파일 = 8-17 표본 — real_ink_reported의 "
        + "좌표 일치 · 12차 두 표본 도착). 실좌표 측정은 `real_ink.k_metrics.per_doc`이 낸다. "
        + "여기의 재현·판정은 보고된 증상의 **코드 회귀 팔**로 남는다."
      ),
      what_this_does_not_say: [
        "**`fov_gate.real_ink_reported*`의 입력은 지시문이 보고한 수다** — 파일 측정이 아니다. "
        + "⚠ 2026-08-19: 파일 셋이 sessions/에 도착해 실좌표 측정은 `real_ink.k_metrics.per_doc`"
        + "이 낸다(보고 f/W와의 대조 포함). 이 블록은 보고값 회귀 팔로 남는다.",
