...
describe('web2-31 3번 — 돋보기', () => {
  ...
  it('작도 시점 갈래 — 종이와 3D의 1:1이 «유지된다» (+반증: 렌즈로 채우면 깨진다)', () => {
    ...
    ledger['draw_branch_lock'] = {
      ...
      note: '⚠⚠ **31-2가 이 예고를 밟았고 답은 「안 쓴다」였다**(2026-08-31 · web2-31 2번 2차 리뷰어 [9] · '
        + '#42 ⑥). 초판의 문장은 「31-2가 보기 렌즈(viewF)를 들이면 그 값이 이 문을 넘는 쪽이다」였는데, '
        + '**보기 렌즈로는 이 문을 넘는 판을 만들 수 없다**: 31-2는 렌즈를 «문서 → 화면» 닮음에 '
        + '합성했으므로(`state.viewXf`) 종이의 2D 잉크와 3D 사영이 **같은 배율로 함께** 움직여 '
        + '1:1이 구성상 유지된다(실측: 렌즈를 바꾼 상태의 화면 왕복·잉크 심판 **0** — `lens31_web2.json` '
        + '`gate4_principle_d`). 여기 남는 719.094는 **해의 f를 바꾼 판**의 값이고 그것이 곧 폐기된 '
        + '「렌즈 슬라이더」다. 그래서 이 팔은 `viewF`를 문으로 **안 쓴다** — 가정 원장의 정정은 AS-C132·AS-C134.',
    }
  })
