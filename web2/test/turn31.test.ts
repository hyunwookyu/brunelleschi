...
describe('web2-31 1번 — 90° 화살표의 틀은 축이다', () => {
  ...
  it('원장', () => {
    // ⚠⚠ **산문이 드는 수는 원장 필드에서 유도한다**(#42 ⑥ · #88 — web2-31 마감 [7]).
    //   초판은 반증 판의 각을 `37.760186`으로 **손으로 적었고** 필드는 `37.761244`였다.
    //   손으로 적으면 픽스처가 바뀔 때 산문만 낡는다 — 그래서 같은 배열에서 읽는다.
    const screenBoardDeg = (ledger['gate3_falsification'] as
      { two_point_rotated_fixture: { screen_board_align_deg: number[] } })
      .two_point_rotated_fixture.screen_board_align_deg
    const SB = screenBoardDeg[0]!.toFixed(6)
    const payload = JSON.stringify({
      ...
      gate: {
        ...
        registered: [
          ...
          `**반증**: 같은 기제에 「화면 기준」 틀을 끼우면 같은 게이트가 ${SB}°로 빨개진다`,
        ...
        reachability: (
          ...
          + `끼우면 게이트 ②의 정렬 각이 0 → **${SB}°**(vp0 기준 최소)로 뛴다. `
      ...
      selfcheck_flags_known: {
        exact_zero_angles: (
          ...
          + `틀만 갈아 끼운 판」이고 그 판은 ${SB}°로 **실제로 넘었다**(gate.falsification_executed — `
