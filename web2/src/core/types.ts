...
export interface Stroke {
  ...
  /** web2-46: 재료 칠이면 셋이 **같이** 있다(m 재료 id · t 톤 인덱스 · i 도구 1=마커
   *  2=색연필). 없으면 45의 붓(흑연) — 옛 파일 그대로다. 셋 중 하나라도 모양이 틀리면
   *  파서가 셋을 함께 버린다(흑연으로 강등 — 조용히 틀린 색을 만들지 않는다). */
  paint?: { f: number; m?: string; t?: number; i?: 1 | 2 }
}
