...
export interface Stroke {
  ...
  lock?: 1
  /** **접합 끊기**(web2-56) — 이 획이 이루는 면 경계 모서리에서 두께 «접합»을 하지
   *  않는다(그 끝이 평평해지고 이음선이 보인다). 값은 1 하나(«있음»이 곧 참 — `lock`의
   *  규격 그대로 · 옛 파일에는 없고 그때는 접합한다). 끝단마다 끊기(지시 — 조사한 모든
   *  툴에 있다: 어떤 알고리즘도 100%를 못 맞춘다)의 이 앱 자리: 벽의 끝단 = 경계
   *  모서리 = 획이다. 저장한다(사람의 결정 — lock과 같은 급). 읽는 자리는 접합 입력
   *  하나다(`state.buildJoints` → `core/joint.ts`). */
  nj?: 1
   ...
   *  web2-48 48-2 → 58-1: `w` = 자국 굵기 px(크기 슬라이더 — 「슬라이더 ⛔」는 R1 오적용으로 철회 · DECISIONS).
   *  `i` = 도구 1=마커 2=색연필. 없으면 45의 붓(흑연) — 옛 파일 그대로다. */
  /** ⚠⚠ **web2-50 — 칠의 정본이 면 위 좌표가 됐다**(「자국의 뿌리」). `uv`는 면 평면
   *  좌표 평탄 배열 [u0,v0,u1,v1,…] — 기저는 `matrep.repBasis`(#54 — 무늬와 같은 기저) ·
   *  단위는 **세계 단위**(mm 아님 — 축척 미정에도 칠한다). 텍스처는 여기서 굽는 파생이다
   *  (`core/facetex.ts` — 원칙 b: 저장 안 한다). `press`는 점별 필압(0..PRESS_Q — 지시의
   *  정본 목록 «압력»· 질은 51의 몫이라 지금은 싣기만 한다). `w`의 단위가 px → **세계
   *  단위**로 바뀌었다(칠은 면 위 안료라 원근을 받는다 — 원칙 e와의 «다름»의 결정).
   *  **uv 없는 칠 획(45~48 형식)은 파서가 획째 버리고 센다** — 사용자 확정 「잃어도
   *  상관없다」(50 지시 · 마이그레이션 ⛔ · 여는 순간 한 줄 알림). */
  /** web2-55: `e: 1` = **테두리 슬롯**의 칠 — uv가 (경계거리 s, 두께 방향 u)다(세계 단위 ·
   *  core/border.ts가 정본). 테두리 획은 s(면의 쪽)가 없다 — 띠는 실제 표면이라 쪽 개념
   *  밖이다(단면이 아니다 — 지시 문면). e 없으면 종전 그대로(앞/뒤 슬롯 · s ±1). */
  /** web2-64: `br` = **브러시 id**(64-1 — 획은 슬롯이 아니라 브러시를 든다. 슬롯(i)은 «성질의 족»(캡·결·기본 색)이고
   *  필통(68 — 옛 여섯 칸)의 자리다) · `o` = 불투명(0..1 · 1이면 안 쓴다 — 옛 문서 바이트 무변) · `c`는 이제 모든 슬롯이 든다
   *  (잉크펜도 색을 쓴다 — 없으면 옛 규약대로 등급 흑연색). 파서는 br 없는 칠 획을 그 슬롯의 «지금» 브러시로 이주하고 센다. */
  /** web2-67 0-6: `er` = **지우개 표식**(1 하나) — 이 획은 «덮임을 빼는 획»이다(eraser=1 브러시로
   *  굽는다 · 순서는 획 목록 그대로라 칠→지우기→칠이 순서대로 빠진다). 없으면 칠(옛 문서 이주 없음). */
  paint?: { f: number; s?: 1 | -1; e?: 1; er?: 1; uv?: number[]; press?: number[]; c?: string; i?: 1 | 2 | 3; w?: number; br?: string; o?: number }
}
...
export interface Doc {
  ...
  persons?: Person[]
  /** **분류 정의의 덮어쓰기**(web2-55) — 분류가 두께·기준·우선순위·코어·재료를 든다
   *  (`core/clsdef.ts`가 정본 — 기본값 위 부분 병합). **바뀐 것만** 담는다: 옛 문서와
   *  t=0 문서는 이 열쇠가 아예 없다(왕복 동일성 — «t=0이면 지금과 픽셀·바이트가 같다»). */
  clsDefs?: Partial<Record<import('./paint').FaceClass, Partial<import('./clsdef').ClsDef>>>
}
...
export interface Face {
  ...
  cls?: 'slab' | 'wall' | 'extw' | 'intw' | 'slope'
  /** **예외**(web2-55) — 이 면만 분류의 값과 다르게. 지금 덮을 수 있는 것은 t(mm)뿐이다
   *  (지시: 「면에는 예외만」). 없으면 분류의 값이다 — 저장도 없을 때 열쇠 없음. */
  ex?: { t?: number }
