...
// 이 거리보다 멀면 «?». 실측으로 놓았다(test/digits.test.ts의 표본):
// 옳은 매칭의 최악 0.061(«2») · 잡음(가로선·지그재그)의 최선 0.1375 → 0.10은
// 위로 64%·아래로 27% 여유다. ⚠ 실필기 표본 0 — 흘려 쓰면 «?»가 나는 쪽으로
// 기운 값이다(조용히 틀린 치수보다 다시 쓰기가 싸다). AS-C24.
const REJECT = 0.10
