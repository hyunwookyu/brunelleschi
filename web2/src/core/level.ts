 ...
 *
 *  **되돌리는 것**: 높이 · 피치 · 롤 · 베어링 — 앵커를 통째로 쓴다.
 *  **새로 쓰는 것**: ① 좌우 각도(요) ② **pivot까지의 거리**. 둘 다 사용자가 정한 것이다.
 *
 *  ⚠⚠ **거리가 왜 «새로 쓰는 것»인가**(web2-06 지시 5) — #60의 물음을 한 겹 더 판 답이다:
 *  **궤도는 pivot 둘레의 회전이므로 거리를 구성상 보존한다**(실측: 궤도 전 7.565 → 궤도 후
 *  7.565). 그러니 거리가 달라졌다면 그것은 **궤도의 부산물일 수 없고** 사람이 줌으로 정한
 *  값이다. 높이는 반대다 — 궤도가 그대로 바꾼다(그것이 #60이 잡은 것).
 *  고치기 전에는 접기가 그 줌을 통째로 지웠다(줌 후 3.782 → **접은 뒤 7.565**).
 *
