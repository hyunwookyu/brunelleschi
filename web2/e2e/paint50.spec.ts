...
test('⚑ 성능 — 스무 면 · 칠 40획: 프레임(#82 — 차)과 텍스처 바이트(tex_budget)', async ({ page }) => {
  ...
  // ⚠ web2-65 §2 ① — 칠 패널이 화면 «왼쪽 가장자리»(x 6..278 · 칠 도구를 든 동안)로 옮겼다.
  // 옛 자리(x 180~477)의 앞 셋이 패널에 먹혀 놓인 획이 30 → 27이 됐다(실측). 패널은 열려
  // 있는 동안 그만큼 덮는 것이 정상이므로(오른쪽에 있을 때도 같았다) **칠 자리만** 오른쪽으로
  // 민다 — 왼쪽 벽(화면 x 150~500)의 오른쪽 몫이라 재는 대상(면·획 수·바이트)은 그대로다.
  for (let i = 0; i < 10; i++) {
    const x = 300 + i * 20
