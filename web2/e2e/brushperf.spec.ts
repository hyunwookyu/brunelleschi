...
test('2-f 원장 — 지연·프레임(그리기/궤도)·재그리기 분산·힙·번들을 두 렌더러 나란히 잰다', async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name === 'dpr1') {
    ...
    writeFileSync(out, JSON.stringify({
      ...
      falsification: '**brush 렌더러 조건의** drawing_*.brush.brush_redraws_while_down이 0이 아니면 캐시 설계가 깨진 것(팔이 0을 강제 — classic 칸의 0은 겹 미사용이라 판정에 안 쓴다) · orbit_500.brush.brush_redraws가 0이면 양성 채널이 죽은 것(팔이 >0 강제) · full_redraw가 전부 0이면 재그리기 미작동(팔이 500행 min>0 강제).',
    }, null, 1))
  }
})
