...
import { PENCIL_FAMILY, CHARCOAL_FAMILY, CHARCOAL_FAMILY_GRADES } from '../src/app/brushes64'
...
const OUT: Record<string, unknown> = {
  ...
  selfcheck_notes: {
    ...
    zero_counters: '보정 «켬»(cal)의 8B/HB 폭 비 ≈ 1은 58 크기 정직성의 뜻 그 자체(요청 폭 == 반최대 폭) — 폭 축의 게이트는 보정 «끔»(raw) 열이 든다(AS-C200)',
...
async function fitOpaque(page: Page, preset: string, target: number, tol: number, maxIter: number, lo = 0.02, hi = 2): Promise<{ opaque: number; reached: number; iters: number; state: string; trace: [number, number][] }> {
...
test('§2 개정 — ① opaque 되먹임(fit) · 굳힌 상수 == 원장 · ② 가족 표 · ③ 단조·8B·폭 비 게이트 · 반증 · 결정론', async ({ page }) => {
  ...
  for (const shape of ['hill', 'line'] as const) {
    ...
    verdict[`pencil_${shape}`] = { density_cal: dCal, density_raw: dRaw, width_raw_24: wRaw, width_cal_24: wCal, mono_density: md, mono_width_raw: mw, ratio_8B_HB_raw: ratioRaw, ratio_8B_HB_cal: ratioCal, width_honest_ratio: honest }
    ...
    if (shape === 'hill') {
      gates.push({ name: `연필 8B/HB 폭 비 ${C.PAINT68_WIDTH_RATIO_8B_HB} ± ${C.PAINT68_WIDTH_RATIO_TOL}(hill 24px 제품)`, ok: Math.abs(ratioCal - C.PAINT68_WIDTH_RATIO_8B_HB) <= C.PAINT68_WIDTH_RATIO_TOL, value: ratioCal })
      ...
      gates.push({ name: `연필 line 8B/HB 폭 비 > 1.5(방향 — 수치 문은 hill)`, ok: ratioCal > 1.5, value: ratioCal })
    }
