...
test('⑩ 3-b(영역 재조립)의 400획 프레임 비용 — 겹 없음/겹+막 나란히 · cost18 참조', async ({ page }, testInfo) => {
  ...
  for (const [nm, q] of [['base_draw', baseDraw], ['base_orbit', baseOrbit], ['with_draw', withDraw], ['with_orbit', withOrbit]] as const)
  ...
  const ratio = (a: number, b: number) => (a > 0 ? +(b / a).toFixed(2) : null)
  const out = {
    what: 'web2-20 3-e ⑩ — 3-b(영역 재조립: #film 사본+곱 · #layerc 몸체)의 400획 프레임 비용. 겹 없음 칸이 same-run 기준, cost18 값은 다른 실행의 참조.',
    run: { project: testInfo.project.name, note: '`npx playwright test cost20 --workers=1` — cost18과 같은 규율. 절대 ms는 컨테이너 변동이 크다(판별은 같은 실행 안의 배수).' },
    scene_at_400: scene,
    frames: {
      base_draw_pose: baseDraw, with_film_draw_pose: withDraw,
      base_orbit: baseOrbit, with_layer_orbit: withOrbit,
    },
    marginal: {
      draw_pose_total_x: ratio(baseDraw!.total, withDraw!.total),
      draw_pose_d2_delta_ms: +(withDraw!.d2 - baseDraw!.d2).toFixed(3),
      orbit_total_x: ratio(baseOrbit!.total, withOrbit!.total),
      orbit_d2_delta_ms: +(withOrbit!.d2 - baseOrbit!.d2).toFixed(3),
  ...
  console.log(`[⑩] draw ${baseDraw!.total.toFixed(2)}→${withDraw!.total.toFixed(2)}ms (${out.marginal.draw_pose_total_x}×) · orbit ${baseOrbit!.total.toFixed(2)}→${withOrbit!.total.toFixed(2)}ms (${out.marginal.orbit_total_x}×)`)
})
