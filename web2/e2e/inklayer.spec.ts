...
async function arm5(page: Page) {
  ...
  ledger.follow = {
    ...
    note: 'off = 사영 선분까지의 **수직거리**(px) — 가장 어두운 칸에서 잰다. #ink의 잉크가 #gl의 사영 모델과 같은 좌표를 쓰면 굵기 반경 대역(≤1.5px)에 머문다. best는 그 칸의 광도(잉크 #101014는 16이고, 1.5px 선의 AA로 70대가 정상이다 — 그 수가 «잉크가 거기 있다»의 값이다). off=null은 그 창에 어두운 칸이 없다는 뜻(궤도로 화면 밖·대기로 떨어짐) — 어긋남과 구별해 적는다. ⚠ 첫 판은 «중점까지의 거리»를 쟀는데, 선을 따라 명도가 거의 같아 동률의 첫 칸(창 가장자리)이 잡혀 12.04가 상수로 나왔다 — 그 지표는 어긋남이 아니라 주사 순서를 쟀다.',
  }
