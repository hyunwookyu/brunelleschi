...
// ⚠⚠ **web2-34 6번이 표를 갈랐다**(#75 ㉣): 「작도 시점으로」가 **맨 위**로 들어왔고
// (가르는 축이 「그리는 중에 쓰는가」다 — 시점 조작은 그리면서 계속 쓴다), 롤 **둘**이
// **롤통 하나**가 됐다(자리를 만드는 것이 목적 · 연필통·촉통·크기통과 같은 문법).
// 이 팔이 지키는 요구(「한 띠 «안»의 순서가 값으로 못 박혀 있다」)는 그대로다.
test('28-4 ④ 손 리본 순서 — 시점 → 자 → 연필 → 펜 → 지우개 둘 → 치수 → 롤통 → 면', async ({ page }) => {
  await boot(page)
  const order = await page.evaluate(() => {
    const want = ['btn-draw-view', 'btn-zoom-fit', 'btn-snap', 'btn-pencil', 'btn-pen', 'btn-eraser-pencil',
  ...
  const sizes = await page.evaluate(() =>
    ['btn-snap', 'btn-pencil', 'btn-pen', 'btn-eraser-pencil', 'dim-toggle', 'btn-roll', 'btn-face', 'btn-draw-view', 'btn-zoom-fit']
