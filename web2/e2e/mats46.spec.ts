...
test('① 칠 패널(64) — 도구를 들면 뜨고 놓으면 사라진다 · 줄 전수가 값으로 · 색은 hex 하나(#88)', async ({ page }) => {
  ...
  // 46 → 48(10) → 51(11) → 52(15) → 54(16) → 62(17) → 64(6) → 67(8) → **68: 일곱** — 브러시 견본 · 크기 ·
  // 불투명 · 색(원 + 최근) · **색상 휠 행**(0-4 — 패널 «안» · 평소 hidden이되 DOM에 있다) · **필통**(68 — 도구 칸 일곱 +
  // 지우개 칸이 «한 줄»의 격자 · 옛 67의 지우개 행이 필통의 여덟째 칸으로 들어갔다 — 칸의 단추는 .favbtn/.ebtn이라 줄 셈 밖) · 정면.
  // 이 팔이 지키는 요구(「자리마다 무엇이 있는지가 값으로 못 박혀 있다」)는 그대로다(68 판갈이).
  expect(rows, '브러시 + 크기 + 불투명 + 색 + 휠 행 + 필통 + 정면(7)').toBe(7)
  expect(await page.locator('#paint-erase').count(), '지우개 고정 칸(67 0-6 → 68 필통 여덟째)').toBe(1)
  expect(await page.locator('#paint-favs > *').count(), '필통 칸 여덟(68 — 도구 7 + 지우개 1)').toBe(8)
