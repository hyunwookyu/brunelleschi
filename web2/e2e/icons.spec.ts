...
test('② 선 아이콘 무채색 — 직접 그리는 도구 밖의 svg에 채도가 없다', async ({ page }) => {
  ...
  const bad = await page.evaluate(() => {
    ...
    for (const svg of document.querySelectorAll('svg')) {
      ...
      // ⚠ **`#painttray`가 들어왔다**(web2-48 48-2) — 이 팔의 머리주석이 이미 적은
      //    그 규칙이다: 「직접 그리는 도구·재료 표현은 밖 — **재료색이 정체다**」.
      //    칠통은 붓·마커·색연필과 **그 굵기의 자국**(48-2 — 지우개 크기통과 같은
      //    문법으로 지은 줄)과 **재료 견본**을 든다 — `#pentray`(30-2)·`#etray`(34-3)가
      //    들어온 것과 **같은 수**이고 예외를 넓힌 것이 아니라 같은 무리에 자리가 하나
      //    늘었다. 걸린 값은 자국 줄의 라벨 글씨 `#3c3831` — 연필통·지우개통이 이미
      //    쓰는 바로 그 라벨 잃크다(새 색 ⛔ · #54).
      if (svg.closest('#btn-pencil, #btn-pen, #tray, #pentray, #etray, #painttray, #oldtools, #btn-eraser-pencil, #btn-eraser-ink, #btn-face, #boot, #btn-roll-yellow')) continue
