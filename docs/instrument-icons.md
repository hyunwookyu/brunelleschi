...
### 종이와 갱신 화살표 — 시점 남기기 (2026-08-31 · web2-31 4번)

지시 문면: 「**종이 아이콘에 갱신 화살표(원형 화살표)를 붙이고 카메라 도형을 뺀다.**
아이콘 문법은 기존 그대로.」 옛것은 종이 탭 띠의 **셔터**였고 Phosphor light `camera`를
`fill="currentColor"`로 **채운** 그림이었다(web2-25 3-a) — 31-3의 돋보기가 들어오면서
둘 다 «광학 기구»가 되어 뜻이 겹쳤고, 문법도 이 파일의 「그 외 전부」와 달랐다
(34-5의 톱니가 같은 자리였다). 그래서 **자작**으로 다시 그린다:
`fill:none · stroke:currentColor · stroke-width 1.6/32 · round cap/join · 닫힌 실루엣`.

- 실루엣은 **종이(닫힌 실루엣) + 접힌 모서리 + 갱신 화살표(호 + 촉)** 넷이다.
  종이는 **가로 12.4 × 세로 16**(x 4.6..17 · y 3.6..19.6 · 비 0.775)이고 오른위 모서리를
  `12.4,3.6 → 17,8.2`로 접는다 —
  접힌 자국이 있어야 «종이»로 읽힌다(사각형 하나는 그냥 사각형이다).
- **화살표는 원 하나가 아니라 «호 + 촉»**이다. 중심 (23.2, 23.2) · 반지름 5.6 ·
  **300° → 265°를 시계 방향으로 325°**(틈 35°)만 돈다. 촉은 끝점 `B`의 접선 방향
  (`d = (−sin265°, cos265°)`)으로 낸 갈매기 둘이고 날개 길이 **3.302**로 같다
  (채운 삼각형 ⛔ — 선 문법이므로 촉도 선이다).
- **틈과 촉을 오른위에 둔 이유**: 종이가 왼위에 있으므로 촉이 왼쪽으로 나가면 종이의
  오른 모서리와 겹친다. 촉의 두 날개 끝 x는 **20.075 · 19.770**이라 종이의 바깥선
  (17 + 선 반폭 0.8 = 17.8)보다 오른쪽이다.
- **붙는다**(지시의 「붙이고」): 호의 중심선이 종이 상자에 가장 가까워지는 자리가
  (18.363, 20.379)이고 그 거리가 **1.569**다 — 두 선의 반폭 합(0.8 + 0.8 = 1.6)과 거의
  같아 **모서리에서 스치듯 붙고 서로를 파고들지 않는다**(겹치는 깊이 0.031).
- 바깥 끝은 23.2 + 5.6 + 0.8 = **29.6**, 왼 끝은 4.6 − 0.8 = **3.8**이라 32 뷰박스 안이다.
- ⚠ **크기 급이 없다** — 이 단추(`#paper-add`)는 종이 탭 띠 안이라 높이를 주는 CSS 급이
  없다. 그래서 여기서만 `width`/`height`를 **16**으로 박고 `vertical-align:-3px`을 둔다:
  **옛 카메라와 같은 상자**라 띠의 자리가 한 픽셀도 안 변한다(#88 — 예약을 안 건드린다).
  정본 블록은 급을 안 싣고, `src/app/paperbar.ts`의 `SHEET_ICON`이 그 둘을 얹는다.
...
⚠ **이름은 「시점 남기기」이지 「시점 갱신」이 아니다**(D-4 · `assumptions.md` AS-C133).
이 단추가 부르는 것은 **`capture` 훅 하나**(그 끝이 `main.ts`의 `captureSheet`)라 **새 장이
는다** — 실측: 한 번 더 누르면 **먼저 만든 장의 좌표 드리프트가 0**이고 장이 하나 더 는다
