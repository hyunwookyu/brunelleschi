...
읽은 증거는 걸리는 번호를 `progress.md`에 적는 것. ⚠ 5차 실증: **#42(완료 시 원장 인용
대조)가 같은 세션에 세 번 재발했다**(이월-2 유령 인용 → 항목 1 #23 유령 → 항목 2 누락 —
전부 리뷰어가 잡았다). **수치를 문서에 적기 전에 원장을 그 자리에서 읽는다** — "0.0000°"
...
| 항목 | 내용 |
|---|---|
| **마지막 리뷰어 재검** | 항목 3~8 리뷰어 호출의 대응 표(progress 맨 끝)를 다음 세션이 재검한다 — 이 세션 마감 시점에 호출이 진행 중이었으면 그 결과 대응부터 |
...
viewpoint_undo·view_cube — **전부 버그 되살림으로 실패 확인**, 되살림 값은 원장 밖 #25 명시) ·
