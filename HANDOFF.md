...
**개입이 닿는 층에서 판정 팔은 바닥이다**(`headline_no_identity` = 1점 제외):
**302획을 놓고 절단 0.2 안 0개 · 0.1 안 0개**, 조용히 틀림이 **정확히 1.0**(대역 0.269).

**반사실이 보인 것과 못 보인 것**(#39·#13·#31·#32): 화면 직교 스냅의 `forced="screen"`이
`stepRule`의 P1 가드를 우회한다(`mainL`의 `snapForced`, 앱과 같은 배선 #17).
끄면 **P1 243 → 181 · P3 33 → 75로 차수는 풀린다.**
⚠⚠ **배치가 나아지는지는 이 하네스로 안 갈린다 — 이 자리에서 두 번 틀렸다.** 표제의
"절단 0.2 안 85 → 85 불변"은 **그 85가 전부 1점 구도의 것**이고 그 구도에서 **두 팔이 같은
실행**이기 때문이다(inside 44/85/419 · P1 60/60까지 동일). 개입이 닿는 네 구도에서는 두 팔
다 **0**(0/302 대 0/545)이라 **0에서는 개선이 관측될 수 없다.** 절단을 0.5로 바꾸면 1 → 3,
형태 중앙은 1.9826 → 1.6092로 **개선 방향**이되 장면 집합이 다르다(n 73 대 108).
⇒ 초판("기여이지 지배항이 아니다")과 정정판("기여 0")을 **둘 다 철회한다.** 말할 수 있는 것은
**"차수는 풀리고 배치 개선 여부는 안 갈린다"**뿐이고, 가르려면 짝지은 부분집합 + 위약 팔이
필요하다(DEFERRED).
...
- ⓐ **모집단 혼합**(리뷰어 [4] · #40 누수 ③): 판정 팔의 조용히 틀림 분모가 구도별로
  ...
  ⇒ **표제 배수를 경로 전체의 대표값으로 인용하지 않는다.**
  ⚠⚠ **유보를 적용하면 판정이 물러지는 게 아니라 세진다**(재검 [R6]): 1점을 뺀 층에서
  **조용히 틀림 3.7배 · 형태 19.5배**로 표제(2.4배·9.4배)보다 크고, 작은 분모 걱정도
  **합산 302획**으로 해소된다 — **네 구도를 통틀어 절단 0.2 안에 놓인 획이 0개**다.
