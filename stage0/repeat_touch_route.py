...
out = {
    "what": "`touch_route` dpr 2 회전 판정(|az − dpr1_az| < 1e-6)을 같은 커밋에서 N회 돌려 "
            "**실패 빈도와 차의 분포**를 남긴다(8차 2차 지시 항목 3). "
    ...
    "by_tol_note": "**임계가 밴드의 몇 배인가**(`ratio_to_band_max`)가 이 표의 읽는 축이다. "
                   ...
                   "9차 항목 3이 고른 **1e-11은 밴드 상단의 약 17.5배**다 — 실패 0이면서 "
                   "dpr 오배선(Δ≈0.5rad)과는 **열한 자리** 여유가 있다. "
                   "⚠ N이 작아 밴드 상단은 **과소추정**이다(#14).",
    "would_fail_at_old_tol_5e_13": {
        "tol": 5e-13,
        "count": sum(1 for x in diffs if x >= 5e-13),
        "denominator": len(diffs),
        "note": "옛 판정(`toBeCloseTo` 정밀도 12)의 임계다. **옛 1e-6은 이 머신의 밴드 "
                "(1e-13대)에서 다섯 자리 밖이라 아무것도 안 갈랐다** — 그래서 0/N이 나왔다. "
